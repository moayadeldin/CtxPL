...
if __name__ == '__main__':
    ...
    if args.pretrained_ckpt is not None:
        try:
                ...
                max_map = checkpoint.get('max_map', max_map)
                print(f"Resumed full checkpoint from iteration {start_itr}")
        except (KeyError, TypeError):
                # Fall back to model-only .pkl
                model.load_state_dict(torch.load(args.pretrained_ckpt))
                print("Loaded model weights only (no optimizer state). Starting from iteration 0.")
                start_itr = 0
